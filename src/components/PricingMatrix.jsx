...
const PricingCard = memo(React.forwardRef(({ tier, isPro, badgeRef }, ref) => {
  ...
  return (
    <div className={`flex flex-col p-8 rounded-2xl border transition-transform duration-200 ease-out hover:-translate-y-2 ${isPro ? 'bg-neural-primary text-neural-noir border-neural-primary shadow-xl shadow-neural-primary/20 scale-105 z-10' : 'bg-neural-noir text-neural-arctic border-neural-teal'}`}>
      <div className="flex justify-between items-start mb-6 min-h-[32px]">
        ...
        {isPro && (
          <span 
            ref={badgeRef} 
            className="font-mono text-xs font-bold bg-neural-noir text-neural-primary px-3 py-1.5 rounded-full border border-neural-noir/20"
            style={{ display: 'none' }}
          >
            SAVE 20%
          </span>
        ...
        <ul className="flex flex-col gap-4 mb-10">
          {features.map((feature, i) => (
            <li key={i} className="flex items-center gap-3 font-sans font-medium opacity-90">
              <span className={isPro ? 'text-neural-noir' : 'text-neural-primary'}>
                <CheckIcon />
              </span>
              {feature}
            </li>
      ...
      <button className={`w-full py-4 rounded-full font-bold transition-all duration-150 ease-out hover:scale-[1.02] ${isPro ? 'bg-neural-noir text-neural-primary hover:bg-black' : 'bg-neural-primary text-neural-noir hover:bg-neural-primary-hover'}`}>
        Get Started
      </button>
    </div>
  );
      ...
      <div className="flex flex-col md:flex-row items-center justify-between w-full max-w-5xl mb-12 gap-8">
        <div className="flex items-center gap-4">
          ...
          >
            <div 
              ref={pillRef} 
              className="w-5 h-5 bg-neural-primary rounded-full transition-transform duration-200 ease-out" 
            />
          </button>
        ...
        <select 
          className="bg-neural-noir text-neural-mint border border-neural-teal rounded-lg px-4 py-2 font-mono text-sm focus:outline-none focus:border-neural-primary appearance-none cursor-pointer outline-none"
